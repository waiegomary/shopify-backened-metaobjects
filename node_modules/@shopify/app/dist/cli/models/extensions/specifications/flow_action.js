...
import { validateFieldShape, startsWithHttps, validateCustomConfigurationPageConfig, validateReturnTypeConfig, } from '../../../services/flow/validation.js';
...
export const FlowActionExtensionSchema = BaseSchemaWithHandle.extend({
    ...
    const configurationPageIsValid = validateCustomConfigurationPageConfig(config.config_page_url, config.config_page_preview_url, config.validation_url);
    const fields = config.settings?.fields ?? [];
    const settingsFieldsAreValid = fields.every((field, index) => validateFieldShape(field, 'flow_action', config.handle, index));
 ...
 */
const loadSchemaPatchFromPath = async (extensionPath, patchPath) => {
    if (!patchPath) {
        return '';
    }
...
//# sourceMappingURL=flow_action.js.map