...
async function loadConfigurationFile(filepath, abortOrReport, 
...
export async function parseConfigurationFile(schema, filepath, abortOrReport, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
decode = decodeToml) {
...
class AppLoader {
    ...
    async loaded() {
        ...
        const { directory: appDirectory, configurationPath, configuration, configurationLoadResultMetadata, } = await configurationLoader.loaded();
        ...
        const appClass = new App(name, 'SHOPIFY_API_KEY', appDirectory, packageManager, configuration, configurationPath, nodeDependencies, webs, allExtensions, usesWorkspaces, dotenv);
    ...
    async loadExtensions(appDirectory, extensionDirectories) {
        ...
        allExtensions.forEach((extension) => {
            if (extension.handle && handles.has(extension.handle)) {
                ...
                this.abortOrReport(outputContent `Duplicated handle "${handle}" in extensions ${result}. Handle needs to be unique per extension.`, undefined, extension.configurationPath);
            }
...
class AppConfigurationLoader {
    ...
    abort(errorMessage, fallback, configurationPath) {
        throw new AbortError(errorMessage);
    }
}
